...
class VmListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        swipeRefresh.setOnRefreshListener {
            getVmList(adapter, item!!.api_url)
        }
...
//        })
    ...
    fun getVmList(adapter: VmListAdapter, url: String){
        ApiService(url).instanceRetrofit.getVmList().enqueue(object: Callback<com.example.cloudraya.ModelApiTest.ResponseVmList>{
            override fun onResponse(
                call: Call<com.example.cloudraya.ModelApiTest.ResponseVmList>,
                response: Response<com.example.cloudraya.ModelApiTest.ResponseVmList>,
            ) {
                ...
                if (respon?.code == 200 ){
                    ...
                    vmList = respon.data as List<DataVm>
                    binding.apply {
                        rvVmList.layoutManager = LinearLayoutManager(this@VmListActivity)
                        rvVmList.adapter = adapter
                        adapter.setVmList(vmList as MutableList<DataVm>)
                    }
                }else {
                    Toast.makeText(this@VmListActivity, "error ${respon?.code}", Toast.LENGTH_SHORT).show()
                }
            }
            override fun onFailure(call: Call<com.example.cloudraya.ModelApiTest.ResponseVmList>, t: Throwable) {
                Log.e("error", t.message!!)
            }
        })
    }
